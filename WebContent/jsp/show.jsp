...
<% 
	if (session.getAttribute("user") == null) {//表示登录信息错误session超时或者为登录
		...
		response.sendRedirect("login.jsp"); //返回到主页界面：即是登陆界面
	}
%>
	...
	<div class="chatMain">
		...
		%>
			<ul class="ulSet">
				...
				<form action="jsp/reply.jsp" method="post" width="50px" height="50px">
					<li class="li1" height="40px">
						<!-- 隐藏表单的使用 -->
				...
				<form action="SeeReplyServlet" method="post" width="50px" height="50px"">
					<li class="li1" height="40px">
						<input type="hidden" name="articleid" value="<%=a.getArticleid()%>"/>
						<input type="hidden" name="userid" value="<%=a.getUserid()%>"/>
